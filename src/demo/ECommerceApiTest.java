...
public class ECommerceApiTest {
	...
	public static void main(String[] args) {
		...
		//pojo class object
			...
			loginRequest.setUserEmail("jinendray1008@gmail.com");
			...
			.param("productSubCategory", "shirts").param("productPrice","1234")
			.param("productDescription", "Feel of fast and durable")
			.param("productFor", "gamers")
			.multiPart("productImage",new File("D:\\Favorities\\lenovo.png"));				//for file attachment we use multipart
			
			String addProductrResponse =  reqAddProduct.when().post("/api/ecom/product/add-product")
			.then().log().all().extract().response().asString();

			JsonPath js = new JsonPath(addProductrResponse);
		   ...
		   String deleteProductRes = deleteProdReq.when().delete("https://rahulshettyacademy.com/api/ecom/product/delete-product/{productId}")
		   .then().log().all().extract().response().asString();
